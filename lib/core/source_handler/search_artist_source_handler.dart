...
class SearchArtistSourceHandler extends SourceHandler {
  ...
  Future<SourceData> getArtistSongData({required String artistId, int page = 1}) async {
    final songs = await _fetchSearchSongsFromNetwork(page: page, artistId: artistId);
    'Artist songs : ${songs.map((e) => e.id).toList()}'.logD;
    return SourceData(songs: songs, sourceType: sourceType, isPaginated: true, currentPage: page);
  }

  Future<List<DbSongModel>> _fetchSearchSongsFromNetwork({required int page, required String artistId}) async {
    if (_hasMoreResult) {
